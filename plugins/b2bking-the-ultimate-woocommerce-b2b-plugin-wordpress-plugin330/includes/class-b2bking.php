	...
	function __construct() {
		...
		if ( wp_doing_ajax() ){
			...
			add_action('plugins_loaded', function(){
				...
				if (get_option( 'b2bking_plugin_status_setting', 'b2b' ) === 'b2b' || (get_option( 'b2bking_plugin_status_setting', 'b2b' ) === 'hybrid' && (get_user_meta( get_current_user_id(), 'b2bking_b2buser', true ) === 'yes' || get_user_meta( $cookieuserid, 'b2bking_b2buser', true ) === 'yes'))){
		...
		add_filter( 'woocommerce_csv_product_import_mapping_options', function( $mappings ){
			...
			foreach ($groups as $group){
				$new_options['b2bking_regular_product_price_group_'.$group->ID] = $group->post_title.__( ' Regular Price', 'b2bking' );
				$new_options['b2bking_sale_product_price_group_'.$group->ID] = $group->post_title.__( ' Sale Price', 'b2bking' );
			}
