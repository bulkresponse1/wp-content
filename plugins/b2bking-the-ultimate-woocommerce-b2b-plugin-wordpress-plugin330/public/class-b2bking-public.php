...
class B2bking_Public{
	...
	function b2bking_payment_method_title( $title, $id ) {
		...
		if (!empty($pmd_rules)){
			...
			if ($maximum !== 'no'){
				if (is_object( WC()->cart )){
					$title.= ' <small>('.get_woocommerce_currency_symbol().' '.esc_html('discount','b2bking').')</small>';
				}
			}
		...
		if (!empty($pmd_rules)){
			...
			if ($maximum !== 'no'){
				if (is_object( WC()->cart )){
					...
					$cart->add_fee( esc_html__($method_title.' discount', 'b2bking'), -$maximum, false);
				}
			}
	...
	function b2bking_handle_file_upload(){
		...
		if( ! in_array(end(explode('.', $_FILES['b2bking_csvorder']['name'])), $allowed_extensions)){
