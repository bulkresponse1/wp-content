...
add_filter('wp_setup_nav_menu_item', 'cyberstore_mikado_add_custom_nav_fields');

// save menu custom fields
add_action('wp_update_nav_menu_item', 'cyberstore_mikado_update_custom_nav_fields', 10, 3);

// edit menu walker
add_filter('wp_edit_nav_menu_walker', 'cyberstore_mikado_edit_walker', 10, 2);

if (!function_exists('cyberstore_mikado_add_custom_nav_fields')) {
    function cyberstore_mikado_add_custom_nav_fields($menu_item) {
        ...
        $menu_item->additional_label = get_post_meta($menu_item->ID, '_menu_item_additional_label', true);
 ...
 * @return      void
 */
if (!function_exists('cyberstore_mikado_update_custom_nav_fields')) {
    function cyberstore_mikado_update_custom_nav_fields($menu_id, $menu_item_db_id, $args) {
        $check = array(
            'anchor',
            'nolink',
            'hide',
 ...
 * @return      void
 */
if (!function_exists('cyberstore_mikado_edit_walker')) {
    function cyberstore_mikado_edit_walker($walker, $menu_id) {
        return 'CyberstoreMikado_Walker_Nav_Menu_Edit_Custom';
    }
}

if (!function_exists('cyberstore_mikado_include_custom_walkers')) {
    function cyberstore_mikado_include_custom_walkers() {
        ...
        do_action('cyberstore_mikado_include_custom_walkers_nav');
    }

    add_action('after_setup_theme', 'cyberstore_mikado_include_custom_walkers');
}