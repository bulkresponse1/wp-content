            ...
            frame: function() {
                ...
                this._frame.on('update', function() {
                    ...
                    jQuery.ajax({
                        type: "post",
                        url: ajaxurl,
                        data: "action=cyberstore_mikado_gallery_upload_get_images&ids=" + ids,
    ...
    */
    function mkdInitAjaxForm() {
        ...
        $(document).delegate(".mkd_ajax_form", "submit", function (a) {
            var b = $(this);
            var c = {
                action: "cyberstore_mikado_save_options"
            };
            jQuery.ajax({
                url: ajaxurl,
                cache: !1,
                ...
                child.find('.mkd-repeater-field').each(function () {
                    ...
                    thisField.find('select').each(function () {
                        ...
                        if(thisInput.hasClass('mkd-select2')) {
                            $('select.mkd-select2').select2("destroy");
                        }
                    });
                });
		...
		if($('.mkd-backup-options-page-holder').length) {
			...
			mkdImportBtn.on('click', function(e) {
				...
				if (confirm('Are you sure, you want to import Options now?')) {
					...
					var data = {
						action: 'cyberstore_mikado_import_theme_options',
						content: importValue,
						nonce: importNonce
					};
					$.ajax({
			...
			mkdImportBtn.on('click', function(e) {
				...
				if (confirm(confirmMessage)) {
					...
					if(import_opt == 'content'){
						for( var i=1; i < 10; i++ ){
							var str;
							if (i < 10) str = 'cyberstore_content_0'+i+'.xml';
							else str = 'cyberstore_content_'+i+'.xml';
							jQuery.ajax({
								type: 'POST',
								url: ajaxurl,
								data: {
