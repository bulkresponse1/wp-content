...
/*
  Class: CyberstoreMikadoIconCollections
...
class CyberstoreMikadoIconCollections
{
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new CyberstoreMikadoIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new CyberstoreMikadoIconsFontElegant("Font Elegant", "fe_icon"));
        $this->addIconCollection('ion_icons', new CyberstoreMikadoIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linea_icons', new CyberstoreMikadoLineaIcons('Linea Icons', 'linea_icon'));
        $this->addIconCollection('linear_icons', new CyberstoreMikadoLinearIcons('Linear Icons', 'linear_icon'));
        $this->addIconCollection('simple_line_icons', new CyberstoreMikadoSimpleLineIcons('Simple Line Icons', 'simple_line_icon'));
        $this->addIconCollection('dripicons', new CyberstoreMikadoDripicons('Dripicons', 'dripicon'));
    }
    ...
    public function getSocialIconsMetaBoxOrOption($attributes) {
        ...
        if ($type == 'meta-box') {
            cyberstore_mikado_create_meta_box_field(
                array(
                    ...
                    'label'         => $label,
                    'options'       => array(
                        'font_awesome' => esc_html__('Font Awesome', 'cyberstore'),
                        'font_elegant' => esc_html__('Font Elegant', 'cyberstore'),
                        'ion_icons'    => esc_html__('Ion Icons', 'cyberstore')
                    ),
                    'args'          => array(
                        'dependence' => true,
        ...
        foreach ($socialIconCollections as $collection_key => $collection_object) {
            ...
            $mkd_icon_pack_container = cyberstore_mikado_add_admin_container(
                array(
                    'parent'          => $parent,
    ...
    public function getVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false) {
        ...
        $iconPackParams = array(
            'type'        => 'dropdown',
            'heading'     => esc_html__('Icon Pack', 'cyberstore'),
        ...
        if (is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach ($this->iconCollections as $key => $collection) {
                $iconSetParams[] = array(
                    'type'        => 'dropdown',
                    'heading'     => esc_html__('Icon', 'cyberstore'),
    ...
    public function getIconWidgetParamsArray() {
        ...
        $iconPackParams[] = array(
            'type'    => 'dropdown',
            'title'   => esc_html__('Icon Pack', 'cyberstore'),
            'name'    => 'icon_pack',
            'options' => array(
                'font_awesome'      => esc_html__('Font Awesome', 'cyberstore'),
                'font_elegant'      => esc_html__('Font Elegant', 'cyberstore'),
                'ion_icons'         => esc_html__('Ion Icons', 'cyberstore'),
                'linea_icons'       => esc_html__('Linea Icons', 'cyberstore'),
                'linear_icons'      => esc_html__('Linear Icons', 'cyberstore'),
                'simple_line_icons' => esc_html__('Simple Line Icons', 'cyberstore'),
                'dripicons'         => esc_html__('Dripicons', 'cyberstore')
            )
        );
    ...
    public function getIconCollectionIcons(iCyberstoreMikadoIconCollection $collection) {
    ...
    public function enqueueStyles() {
        if (is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach ($this->iconCollections as $collection_key => $collection_obj) {
                wp_enqueue_style('mkd-' . $collection_key, $collection_obj->styleUrl);
            }
        }
    }
...
if (!function_exists('cyberstore_mikado_activate_theme_icons')) {
    function cyberstore_mikado_activate_theme_icons() {
        global $cyberstore_IconCollections;
        $cyberstore_IconCollections = CyberstoreMikadoIconCollections::get_instance();
    }

    add_action('after_setup_theme', 'cyberstore_mikado_activate_theme_icons');
}